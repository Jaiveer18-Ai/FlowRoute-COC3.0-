...
type CursorVariant = 'default' | 'button' | 'nav' | 'node' | 'edge' | 'route';
...
const Cursor: React.FC = () => {
  ...
  // Outer ring follows with deliberate subtle lag
  const ringSpringConfig = { damping: 28, stiffness: 260, mass: 0.2 };
  ...
  const getRingStyles = () => {
    switch (cursorVariant) {
      case 'button':
        return {
          size: 38,
          borderColor: 'rgba(22, 119, 255, 0.45)',
      ...
      case 'node':
        return {
          size: 32,
          borderColor: 'rgba(22, 119, 255, 0.65)',
  ...
  const getDotStyles = () => {
    switch (cursorVariant) {
      ...
      case 'node':
        return { size: 3.5, color: 'var(--color-accent)' };
      case 'edge':
        return { size: 3.5, color: 'var(--color-danger)' };
      default:
        return { size: 3.5, color: '#0F172A' };
    }
  };
    ...
    >
      ...
      <motion.div
        style={{
          ...
          borderRadius: cursorVariant === 'node' ? '3px' : '50%',
      ...
      >
        {cursorVariant === 'node' && (
          <div style={{ position: 'absolute', inset: 0 }}>
            <span style={{ position: 'absolute', top: -3, left: '50%', width: 1, height: 3, background: 'var(--color-accent)', transform: 'translateX(-50%)' }} />
            <span style={{ position: 'absolute', bottom: -3, left: '50%', width: 1, height: 3, background: 'var(--color-accent)', transform: 'translateX(-50%)' }} />
            <span style={{ position: 'absolute', left: -3, top: '50%', height: 1, width: 3, background: 'var(--color-accent)', transform: 'translateY(-50%)' }} />
            <span style={{ position: 'absolute', right: -3, top: '50%', height: 1, width: 3, background: 'var(--color-accent)', transform: 'translateY(-50%)' }} />
          </div>
        )}
      </motion.div>
